...
<p align="center">
  <img src="logo.png" alt="LabeloxAV" width="420">
</p>
...
![The home dashboard with a real fleet ingested](screenshots/00-home.png)
...
![Fleet analytics](screenshots/01-analytics.png)
...
![The application menu bar, with the File menu and its Import submenu open](screenshots/menus.gif)
...
![The File menu with the Import submenu open, showing the fourteen supported import formats](screenshots/32-menubar.png)
...
![The platform launcher: seven planes of the data engine, in flywheel order, each described](screenshots/20-ui-platforms.png)
...
![The adaptive flywheel controller: signals in, budget, and the cycle ledger](screenshots/22-ui-flywheel.png)

![Hardware in the loop deployment: co-optimization, thermal envelope, and staged rollout](screenshots/21-ui-deploy-tool.png)
...
![Annotation canvas on a real Indian street, 55 objects on one frame](screenshots/07-annotation-canvas.png)
...
![The cuboid annotation workspace with a real KITTI scan: 3D view above, bird's eye view below](screenshots/11-lidar-inapp.png)
...
![The LiDAR viewer on real KITTI scans: 3D view, bird's eye view, and the cloud list](screenshots/lidar-viewer.gif)
...
![The LiDAR viewer showing a KITTI scan in 3D and bird's eye view simultaneously](screenshots/42-lidar-viewer.png)
...
![Raw height-coloured returns beside the RANSAC ground and obstacle split](screenshots/41-lidar-segmentation.png)
...
![Bird's eye view of three successive KITTI scans, raw and ground-segmented](screenshots/lidar.gif)
...
![The Explore workspace: facet rail on the left with live counts over the ingested corpus](screenshots/30-explore.png)
...
![The embedding cluster map on the analytics page: UMAP over DINOv3 frame embeddings](screenshots/37-cluster-map.png)
...
![Priority queue](screenshots/03-triage.png)
...
![Active learning value queue](screenshots/06-review-queue.png)
...
![The Projects board: jobs by stage and state, assignment, and annotator scorecards](screenshots/31-projects.png)
...
![Model versus gold: per-cell true positives, false positives and misses against a sealed gold set](screenshots/35-eval-drilldown.png)
...
![Text and NER labeling: spans coloured by label, with the project's declared labels and typed fields](screenshots/34-multimodal-text.png)
...
![Governance console](screenshots/02-govern.png)
...
![LabeloxSec static-camera scene model: background prior and foreground detection](screenshots/50-labeloxsec-static-camera.png)
...
![LabeloxSec ANPR-India: plate parsing and the pack capability gate](screenshots/51-labeloxsec-anpr.png)
...
[`docs/DEPLOY.md`](DEPLOY.md) for GPU, serving to other machines, TLS, upgrades, and backups.
...
**Exercised on real data.** A fleet of real dashcam drives is ingested: 186 trips and 32,455 frames from Indian roads, with faces and plates blurred before anything reaches storage. That operational fleet has now been auto-labeled end to end: **32,153 of its 32,455 frames (99.1%) carry detections**, and every object the detectors produced has a DINOv3/SigLIP2 embedding, so find-similar and the embeddings map run on the whole fleet rather than a public-dataset sliver. Corpus-wide that is now **567,515 of 714,415 (79.4%)**, down from 100%, because the track-gap fill described below added 138,024 interpolated boxes that no detector ever saw and the continuous embedder has not reached them. Counting everything in the database, that is 41,752 frames and 714,415 objects, after the fixture purge described below removed 5,245 frames and 13,172 objects that were never real. Semantic segmentation models run drivable surface and lane geometry across it, locally on an RTX 5080 and through the cloud GPU seam that starts a pod, runs the sweep, ingests the result and stops the pod to cap billing. Drivable area now covers **41,718 of 41,752 frames (99.92%)**, and the main 1920x1080 dashcam capture is at 100.0%; it was 3.9% and 0.2% respectively, for the reason recorded below. A real KITTI LiDAR scan is annotated to 3D cuboids and exported as nuScenes. Detectors in the registry are trained on the India Driving Dataset. The explorer, the faceted counts, bulk tagging, similarity search reranked for diversity, the job and honeypot workflow, the model-versus-gold drill-down, dataset export (a DASHCAM-01 fleet slice sealed to a versioned COCO/YOLO commit), the CVAT and Label Studio round trips, and signed webhook delivery have each been driven end to end against this running stack.
...
**More measurement defects, found by auditing and fixed in the open.** The prediction-plane correction above was not the only one. A four-part audit of the codebase turned up several more places where a number existed but did not mean what it appeared to, and they are recorded with their fixes in [`docs/REMEDIATION_STATUS.md`](REMEDIATION_STATUS.md). The ones worth naming here:
...
**Three features that were built, reachable, and had never run.** Each was found the same way: by asking what a number in the database should look like if the feature worked, and then looking. None of them threw an error, and none of them was visible from the interface.

- **Drivable area was segmented on 3.9% of the corpus, and 0.2% of the main capture.** `segment_drivable` had exactly one caller in the tree, the editor's button, so a mask existed only where somebody had opened a frame and clicked. That was not neutral: lane proposals are filtered against the mask, and the filter treats an *absent* mask as plausible, so the lane plausibility gate was wired up and doing nothing on 96% of frames. Turning it on would have produced forty thousand masks nothing could read, because the training reader ran `cv2.imdecode` on a JSON polygon blob and got `None` (the "drivable" source had therefore contributed zero labels to every dataset ever built from it, while `data.yaml` went on declaring three surface classes), the BDD export copied that JSON into files named `.png`, and the local model caught OOM into a silent perspective trapezoid with no VRAM guard. All three fixed first; coverage is now **41,718 of 41,752 (99.92%)** with the main 1920x1080 capture at 100.0%, and 0 lane frames lack a mask. 34 frames are deliberately left without a row rather than given a zero, because they are unreadable test pollution and a zero would read as "checked, no road here".

- **A class correction stopped at the frame it was made on.** 413 tracks carried an unambiguous human class, and of the 44,097 objects on them only 5,798 had it: the median track is 93 frames and the median number of frames a person actually touched is **1**, so 86.9% of every correction ever made sat on one frame while the other 92 kept the detector's guess. The endpoint that fixes a whole track already existed and the frame editor never called it. It also could not safely be called: it wrote the request body's state straight onto the object with a default of `accepted`, making it a third review path around the QA clamp that `services/review_policy.py` documents as universal, and it never advanced the lock version, never recorded a revertible batch, never revalidated attributes and never guarded the class move. That loop now lives in one place shared with bulk review. The backfill carried 13,698 decisions outward, held 14 objects a person had already ruled on, and **refused 24,581** where the ontology guard says the move changes what kind of thing something is: one refused track holds sedans, poles, pedestrians and trucks across 671 frames, which is an ID-switch pileup rather than one object.

- **84% of tracks blinked out and came back.** 9,460 of 11,287 tracks had holes in them, 137,960 frames, which is what "the object was there and now it is gone" looks like from the annotator's side. The holes are short: on the worst track, 392 missing frames arrive as 241 separate holes averaging 1.6 frames and never longer than 5. `interpolate_track` exists for exactly this and had **never run once**, because it wrote `source="interp"` and `ck_object_source` admits nine values that do not include it, so every call raised a check violation. Fixing that string was not enough to run it at scale: it created objects with no run id and nothing to undo, and it drew a straight line through a hole of any length, which on a turning vehicle leaves the road. Holes are now refused past twelve frames and counted, the sweep is one revertible run, and gaps went from **137,960 to 13**. The boxes land at `state=annotate, conf=0.5`, because a linear guess is not a measurement, and they sit exactly 0.00px off the midpoint of their neighbours where real detections are 317px off.

The interpolated boxes are the reason the embedding figure above is no longer 100%, and they are excluded from nothing else automatically: a dataset build that should not train on interpolated geometry has to say so.
...
| --- | --- |
| [`docs/ARCHITECTURE.md`](ARCHITECTURE.md) | The map: the Session/Frame/Object spine, the two planes and why annotation and model output never share a row, the closed loop end to end, the domain-pack split, and the auth model. |
| [`docs/MEASUREMENT.md`](MEASUREMENT.md) | How a metric is produced and why it can be trusted. Read this before touching anything that produces or consumes a model number; it is the document that would have caught the prediction-plane defect. |
| [`docs/DEPLOY.md`](DEPLOY.md) | Installing and running it with Docker: the one-command installer, signing in, serving to other machines, TLS, GPU, upgrades, and backups. |
| [`docs/RUNBOOK.md`](RUNBOOK.md) | Operating it: bring-up, auth bootstrap, the measurement-and-promotion sequence, the test tiers, and troubleshooting. |
