...
FILLERS: dict[str, str] = {
    ...
    "cycle_id": "select distinct provenance->'flywheel'->>'cycle_id' from object"
                " where provenance->'flywheel'->>'cycle_id' is not null limit 1",
}
...
async def sweep(base: str, token: str | None, timeout: float, out_path: Path) -> int:
    ids = await fill()
    ...
    async with httpx.AsyncClient(base_url=base, timeout=timeout, headers=headers) as c:
        ...
        rs = routes(spec)
        results = []
        for i, (path, _op) in enumerate(rs, 1):
            ...
            try:
                ...
                # A 4xx is a working route answering about data that is not there, or a guard doing its
                # job. Only a 5xx, a timeout or an unparseable body is this sweep's business.
...
def main() -> None:
    ...
    a = ap.parse_args()
    raise SystemExit(1 if asyncio.run(sweep(a.base, a.token, a.timeout, Path(a.out))) else 0)
